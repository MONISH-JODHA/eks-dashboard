...
# --- FIX: Increase connection pool sizes for Boto3 and K8s client ---
# This configuration is crucial for handling high concurrency and avoids "Connection pool is full" errors.
BOTO_CONFIG = Config(
...
def get_session(role_arn=None):
    ...
    if not role_arn:
        try:
            # Apply the custom config to the client used for credential validation
            boto3.client('sts', config=BOTO_CONFIG).get_caller_identity()
    ...
    try:
        # Apply the custom config to the STS client for assuming roles
...
def get_k8s_api_client(cluster_name, cluster_endpoint, cluster_ca_data, region, role_arn=None):
    ...
    # FIX: Increase the K8s client's thread pool to match Boto3's concurrency
    cfg.pool_threads = 50
...
def fetch_ecr_vulnerabilities(image_uris, role_arn=None):
    session = get_session(role_arn)
    if not session: return {"error": "Failed to get session for vulnerability scan."}
    try:
        session_account_id = session.client('sts', config=BOTO_CONFIG).get_caller_identity()['Account']
    except Exception as e:
        logging.error(f"Could not determine AWS Account ID for session. Vulnerability scanning will fail. Error: {e}")
        return {uri: {"status": "ERROR", "error": "Could not determine account ID."} for uri in image_uris}
    scan_results = {}
    ecr_clients = {}
    ...
    for uri in image_uris:
        ...
        repo_account_id = parts['aws_account_id']
        if repo_account_id != session_account_id:
            logging.info(f"Skipping scan for external/public image: {uri}")
            scan_results[uri] = {"status": "EXTERNAL_IMAGE", "summary": {}, "findings": []}
            continue
        repo_region = parts['aws_region']
        repository_name = parts['repository_name']
        image_identifier = {'imageDigest': parts['image_digest']} if parts.get('image_digest') else {'imageTag': parts.get('image_tag', 'latest')}
        try:
            if repo_region not in ecr_clients:
                ecr_clients[repo_region] = session.client('ecr', config=BOTO_CONFIG, region_name=repo_region)
            ecr_client = ecr_clients[repo_region]
            paginator = ecr_client.get_paginator('describe_image_scan_findings')
            page_iterator = paginator.paginate(repositoryName=repository_name, imageId=image_identifier, maxResults=1000)
            all_findings, scan_status, findings_summary = [], None, {}
            for page in page_iterator:
                if not scan_status:
                    scan_status = page.get('imageScanStatus', {}).get('status', 'UNKNOWN')
                    findings_summary = page.get('imageScanFindingsSummary', {}).get('findingSeverityCounts', {})
                all_findings.extend(page.get('imageScanFindings', {}).get('findings', []))
            scan_results[uri] = {"status": scan_status, "summary": findings_summary, "findings": all_findings}
        except ClientError as e:
            if e.response['Error']['Code'] in ['ScanNotFoundException', 'ImageNotFoundException', 'RepositoryNotFoundException']:
                status_code = e.response['Error']['Code']
                logging.warning(f"ECR scan status for {uri}: {status_code}")
                scan_results[uri] = {"status": status_code, "summary": {}, "findings": []}
            else:
                logging.error(f"Error fetching ECR scan for {uri}: {e}")
                scan_results[uri] = {"status": "ERROR", "error": str(e), "findings": []}
    return scan_results
...
def get_kubernetes_cluster_objects(cluster_name, cluster_endpoint, cluster_ca, region, role_arn=None):
    ...
    k8s_data = { "pods": [], "services": [], "ingresses": [], "nodes": [], "deployments": [], "events": [], "roles": [], "clusterroles": [], "rolebindings": [], "clusterrolebindings": [], "map_nodes": [], "map_edges": [], "error": None }
    try:
        ...
        vulnerability_results = fetch_ecr_vulnerabilities(list(ecr_image_uris), role_arn)
        map_nodes, map_edges = [], []
        for pod in k8s_data["pods"]:
            pod['vulnerabilities'] = {}
            ...
            details = { "kind": "Pod", "name": pod["metadata"]["name"], "namespace": pod["metadata"]["namespace"], "status": pod["status"]["phase"], "pod_ip": pod["status"].get("podIP", "N/A"), "node_name": pod["spec"].get("nodeName", "N/A"), "restarts": pod['restarts'], "age": pod['age'], "controlled_by": f"{owner_ref.get('kind', 'N/A')}/{owner_ref.get('name', 'N/A')}", "created": pod['metadata']['creationTimestamp'], "vulnerabilities": pod['vulnerabilities']}
...
def fetch_addons_for_cluster(eks_client, cluster_name):
    ...
    try:
        ...
        addon_names = [name for page in paginator.paginate(clusterName=cluster_name) for name in page.get('addons', [])]
        with ThreadPoolExecutor(max_workers=20) as executor:
            future_map = {executor.submit(eks_client.describe_addon, clusterName=cluster_name, addonName=name): name for name in addon_names}
            for future in as_completed(future_map):
                addon_name = future_map[future]
                try:
                    addon_desc = future.result().get('addon', {})
                    if addon_desc:
...
def upgrade_nodegroup_version(account_id, region, cluster_name, nodegroup_name, role_arn=None):
    ...
    try:
        eks_client = session.client('eks', config=BOTO_CONFIG, region_name=region)
...
def get_cluster_metrics(account_id, region, cluster_name, role_arn=None):
    ...
    cw_client = session.client('cloudwatch', config=BOTO_CONFIG, region_name=region)
    metric_definitions = {
        ...
        "pod_network_rx_bytes": ('pod_network_rx_bytes', 'Average'), "pod_network_tx_bytes": ('pod_network_tx_bytes', 'Average'),
        "apiserver_request_total": ('apiserver_request_total', 'Sum'), "apiserver_request_total_4xx": ('apiserver_request_total_4XX', 'Sum'),
        "apiserver_request_total_5xx": ('apiserver_request_total_5XX', 'Sum'), "apiserver_request_latency_get": ('apiserver_request_latencies_GET', 'Average'),
        ...
        "apiserver_inflight_requests_readonly": ('apiserver_in_flight_requests_readonly', 'Average'), "scheduler_schedule_attempts_success": ('scheduler_schedule_attempts_success', 'Sum'),
        "scheduler_schedule_attempts_failure": ('scheduler_schedule_attempts_failure', 'Sum'), "scheduler_schedule_attempts_error": ('scheduler_schedule_attempts_error', 'Sum'),
...
def _process_cluster_data(c_raw, with_details=False, detail_results=None):
    ...
    cluster_data = {
        ...
        "certificateAuthority": c_raw.get("certificateAuthority"),
    }
    if with_details and detail_results:
        ...
        cluster_data.update({
            ...
            # FIX: Add the missing 'networking' key to prevent the Jinja2 error.
            "networking": c_raw.get("resourcesVpcConfig", {}),
        })
    return cluster_data

def get_live_eks_data(user_groups, group_map_str):
    ...
    try:
        primary_account_id = boto3.client('sts', config=BOTO_CONFIG).get_caller_identity().get('Account')
    ...
    with ThreadPoolExecutor(max_workers=30) as executor:
            ...
            for region in [r.strip() for r in os.getenv("AWS_REGIONS", os.getenv("AWS_DEFAULT_REGION", "us-east-1")).split(',') if r.strip()]:
                try:
                    ...
                    for name in cluster_names:
                        ...
                        describe_futures[future] = {'region': region, 'eks_client': eks_client}
        ...
        for future in as_completed(describe_futures):
            context = describe_futures[future]
            try:
                desc = future.result().get('cluster', {})
                if desc:
                    desc['region'] = context['region']
                    desc['eks_client'] = context['eks_client'] # Pass client for security insights
...
def get_single_cluster_details(account_id, region, cluster_name, role_arn=None):
    ...
    try:
        ...
        detail_results = {"role_arn": role_arn, "eks_client": eks_client}
